...
def user_model(ns):
    return ns.model(
        ...
        {
            "id": fields.String(
                readOnly=True, description="Unique user identifier (UUID)"
            ),
...
def user_create(ns):
    return ns.model(
        "UserCreate",
        {
            ...
            "is_admin": fields.Boolean(
                description="Optional admin flag; honored for the very first signup, otherwise only by existing admins"
            ),
        },
    )
...
def user_update(ns):
    return ns.model(
        "UserUpdate",
        {
            ...
            "is_admin": fields.Boolean(
                description="Admin flag; only considered if requester is admin"
            ),
        },
    )
...
def host_model(ns):
    return ns.model(
        ...
        {
            "id": fields.String(
                readOnly=True, description="Unique host identifier (UUID)"
            ),
            ...
            "is_admin": fields.Boolean(
                required=True, description="Flag indicating admin privileges"
            ),
