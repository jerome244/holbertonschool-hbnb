...
ns = Namespace("reviews", description="Review management", security="BearerAuth")
...
review_input = ns.model(
    ...
    {
        "booking_id": fields.String(
            required=True, description="UUID of the booking to review"
        ),
...
class ReviewList(Resource):
    ...
    def post(self):
        ...
        claims = get_jwt()
        if str(booking.user.id) != str(current_user) and not claims.get(
            "is_admin", False
        ):
...
class ReviewDetail(Resource):
    ...
    def put(self, review_id):
        ...
        payload = {"booking_id": booking.id, "text": text, "rating": rating}
