...
@pytest.fixture(scope="module")
def host_id(client):
    rv = client.post('/api/v1/hosts/', json={"first_name": "Host", "last_name": "User", "email": "host@example.com"})
    if rv.status_code != 201:
...
def place_id(client, user_id, amenity_id):
    rv = client.post('/api/v1/places/', json={
        "name": "EdgePlace", "description": "Edge desc",
...
@pytest.mark.parametrize('endpoint,id_', [
    ('users', 999), ('hosts', 999), ('amenities', 999),
    ('places', 999), ('bookings', 999), ('reviews', 999)
])
...
# --- PATCH Tests ---
@pytest.mark.parametrize('endpoint,field,value,ok', [
...
def test_booking_guest_bounds(client, user_id, place_id, guests):
    rv = client.post('/api/v1/bookings/', json={
        "user_id":user_id, "place_id":place_id,
        "guest_count":guests, "checkin_date": date.today().isoformat(), "night_count":1
    })
...
def test_review_rating_bounds(client, booking_id, rating):
    rv = client.post('/api/v1/reviews/', json={"booking_id":booking_id, "text":"X", "rating":rating})
    assert rv.status_code == 400

# --- DELETE & PATCH Review ---
def test_delete_and_patch_review(client, booking_id):
    rv = client.post('/api/v1/reviews/', json={"booking_id":booking_id, "text":"T", "rating":3})
    if rv.status_code != 201:
        pytest.skip()
    rid = rv.get_json()['id']
...
@pytest.mark.parametrize('capacity,price,expected',[
    # current API rejects capacity so expecting 400
    (3,100.0,400),(-1,100.0,400),('big',100.0,400),(2,-5,400),(2,'free',400)
])
def test_place_capacity_price(client, user_id, amenity_id, capacity, price, expected):
    payload={
        "name":"X","description":"D","user_id":user_id,
        "amenity_ids":[amenity_id],"capacity":capacity,"price":price
    }
...
@pytest.mark.parametrize('body,expected',[
    ({"user_id":user_id,"place_id":place_id,"guest_count":1,"checkin_date":date.today().isoformat(),"night_count":2,"price_per_night":50},201),
    ({"user_id":user_id,"place_id":place_id,"guest_count":1,"checkin_date":"2025-99-99","night_count":2,"price_per_night":50},400),
    ({"user_id":user_id,"place_id":place_id,"guest_count":1,"checkin_date":date.today().isoformat(),"night_count":0,"price_per_night":50},400),
    ({"user_id":user_id,"place_id":place_id,"guest_count":1,"checkin_date":date.today().isoformat(),"night_count":2,"price_per_night":-10},400)
])
...
def test_place_average_rating(client, place_id, user_id, amenity_id):
    vals=[]
    for r in (4,2):
        rvb = client.post('/api/v1/bookings/', json={"user_id":user_id,"place_id":place_id,"guest_count":1,"checkin_date":date.today().isoformat(),"night_count":1})
        if rvb.status_code!=201: pytest.skip()
        bid=rvb.get_json()['id']
        rvr=client.post('/api/v1/reviews/', json={"booking_id":bid,"text":"A","rating":r})
        if rvr.status_code!=201: pytest.skip()
        vals.append(r)
