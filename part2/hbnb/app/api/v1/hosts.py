...
host_model = ns.model(
    "Host",
    {
        "id": fields.String(
            readonly=True,
            description="Unique host identifier (UUID)"
        ),
        ...
        "is_admin": fields.Boolean(
            required=True,
            description="Flag indicating if host has admin privileges"
        ),
    }
)
...
class HostList(Resource):
    ...
    @ns.doc('create_host', description='Create a new host. Payload: first_name, last_name, email, optional is_admin')
...
class HostDetail(Resource):
    ...
    @ns.doc('replace_host', description='Replace an existing host completely. Payload: first_name, last_name, email, is_admin')
    ...
    def put(self, host_id):
        ...
        if not EMAIL_RE.match(email):
            ns.abort(400, "Invalid email address")
        if any(h.email.lower() == email and h.id != host_id for h in facade.list_hosts()):
...
class HostOwnedPlaces(Resource):
    ...
    """
    @ns.doc('list_host_owned_places', description='List all places owned by the given host')
