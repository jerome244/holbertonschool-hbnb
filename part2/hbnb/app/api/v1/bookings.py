...
booking_input = ns.model(
    "BookingInput",
    {
        ...
        "place_id": fields.String(
            required=True,
            description="UUID of the place being booked"
        ),
        ...
        "night_count": fields.Integer(
            required=True,
            description="Number of nights; must be > 0"
        ),
    }
)
...
booking_output = ns.inherit(
    ...
    booking_input,
    {
        ...
        "checkout_date": fields.Date(
            readOnly=True,
            description="Computed check-out date"
        ),
    }
)
...
rating_output = ns.model(
    "BookingRating",
    {
        ...
        "rating": fields.Float(
            readOnly=True,
            description="User’s rating for this booking (1–5)"
        ),
    }
)
...
class BookingList(Resource):
    ...
    """
    @ns.doc('list_bookings', description='Retrieve all bookings with computed total price and checkout date')
...
class BookingDetail(Resource):
    ...
    @ns.doc('replace_booking', description='Replace an existing booking completely with validations')
    @ns.expect(booking_input, validate=True)
    @ns.marshal_with(booking_output)
    def put(self, booking_id):
        ...
        b.checkin_date = datetime.combine(date.fromisoformat(data["checkin_date"]), datetime.min.time())
        return {
            "id": b.id,
            "user_id": b.user.id,
            "place_id": b.place.id,
